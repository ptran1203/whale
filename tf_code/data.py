...
def data_augment(config, posting_id, image, label_group, matches):
    ...
    if config.augname == 'normal':
        ...
        image = random_rot_shear(image, rot_limit=15, shear_limit=10,)
...
def decode_image(image_data, box, config):
    ...
    if box is not None and box[0] != -1:
        left, top, right, bottom = (box[3] + tf.cast(O0, tf.int32),
                                    box[2] + tf.cast(O1, tf.int32),
                                    box[1] + tf.cast(O2, tf.int32),
                                    box[0] + tf.cast(O3, tf.int32))
...
def read_labeled_tfrecord(config, is_train, example):
    LABELED_TFREC_FORMAT = {
        ...
        "yolov5_box2": tf.io.FixedLenFeature([4], tf.int64),
